...
template <typename T> using Stack = pointer<T>;
 ...
 */
template <typename T> Stack<T> createStack(Stack<T> Top) {
  Top = nullptr;
  return Top;
}
 ...
 */
template <typename T> void createElement(pointer<T> &newElement, T data) {
 ...
 */
template <typename T> T peek(Stack<T> Top) { return Top->data; }
 ...
 */
template <typename T> bool isEmpty(Stack<T> Top) { return Top == nullptr; }
 ...
 * @return            pointer ke puncak stack
 */
template <typename T> Stack<T> push(Stack<T> Top, pointer<T> newElement) {
  if (isEmpty(Top)) {
    Top = newElement;
  } else {
    newElement->next = Top;
 ...
 */
template <typename T> Stack<T> pop(Stack<T> &Top, pointer<T> delElement) {
  if (isEmpty(Top)) {
    delElement = nullptr;
...
/**
 * @brief Mengambil element terdalam / element dasar / element terakhir pada
 * stack
 ...
 */
template <typename T> Stack<T> lastNode(Stack<T> Top) {
  pointer<T> pHelp = Top;
  if (isEmpty(Top)) {
    // do nothing
  } else {
